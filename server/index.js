...
io.on("connection", (socket) => {
  ...
  socket.on("start-game", ({ roomCode }) => {
    ...
    // Tüm yer ve roller eşleşmesini topla
    const allLocationRoles = room.players.flatMap(p =>
      p.locations.map((loc, i) => ({
        location: loc,
        roles: p.roles.slice(i * 5, i * 5 + 5)
      }))
    );

    // Rastgele bir yer ve o yere ait rollerden biri seç
    const selected = allLocationRoles[Math.floor(Math.random() * allLocationRoles.length)];
    const chosenLocation = selected.location;
    const chosenRole = selected.roles[Math.floor(Math.random() * selected.roles.length)];
    const spyIndex = Math.floor(Math.random() * room.players.length);

    // Oyunculara roller dağıt
